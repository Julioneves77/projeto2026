...
async function fetchWithAuth(url: string, options: RequestInit = {}): Promise<Response> {
  ...
  if (SYNC_SERVER_API_KEY) {
    ...
    console.log('🔑 [Pagar.me] API Key configurada:', SYNC_SERVER_API_KEY.substring(0, 10) + '...');
  } else {
    console.warn('⚠️ [Pagar.me] API Key não configurada');
  }
  
  console.log('📤 [Pagar.me] Headers da requisição:', {
    'Content-Type': headers.get('Content-Type'),
    'X-API-Key': headers.get('X-API-Key') ? 'Configurado' : 'Não configurado',
  });
...
export async function createPixTransaction(
  ...
  try {
    ...
    console.log('📤 [Pagar.me] Payload enviado:', {
      ...payload,
      customer: {
        ...payload.customer,
    ...
    console.log('📥 [Pagar.me] Resposta recebida:', {
    ...
    if (!response.ok) {
      ...
      console.error('❌ [Pagar.me] Erro ao criar transação:', {
        status: response.status,
        statusText: response.statusText,
        errorData,
      });
